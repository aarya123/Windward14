...
public class Framework implements IPlayerCallback {
	...
	private String ipAddress = "k9-00.cs.purdue.edu";

	private String myGuid;

	// we play a card and remove it from our hand. But at the same time the
	// server is sending us a status with our
	// hand as it sees it. So we ignore updates to our hand if it's been less
	// than a second and we're not seeing the
	...
	// Player/Passenger lists, sending
	// back multiple orders, etc. This is a lousy way to handle this - but it
	// keeps the example simple and
	...
	private Framework(java.util.List<String> args) {
		...
		String msg = String.format("Connecting to server %1$s for user: %2$s",
				ipAddress, brain.getName());
	...
	private void Run() throws IOException {
		...
		while (true) {
			try {
				...
				System.out.println("ERROR restarting app (Exception: "
						+ e.getMessage() + " )");
	...
	public final void IncomingMessage(String message) throws DocumentException {
		try {
			...
			// get the xml - we assume we always get a valid message from the
			// server.
			...
			if (rootName.equals("setup")) {
				...
				java.util.ArrayList<Player> players = Player.FromXml(xml
						.getRootElement().element("players"));
				java.util.ArrayList<Company> companies = Company.FromXml(xml
						.getRootElement().element("companies"));
				java.util.ArrayList<CoffeeStore> stores = CoffeeStore
						.FromXml(xml.getRootElement().element("stores"));
				java.util.ArrayList<Passenger> passengers = Passenger.FromXml(
						xml.getRootElement().element("passengers"), companies);
				java.util.ArrayList<PowerUp> powerups = PowerUp.FromXml(xml
						.getRootElement().element("powerups"), companies,
						passengers);

				Map map = new Map(xml.getRootElement().element("map"),
						companies);
				...
				brain.Setup(map, me2, players, companies, stores, passengers,
						powerups, new PlayerAIBase.PlayerOrdersEvent() {
							...
							public void invoke(PlayerAIBase.CARD_ACTION action,
									PowerUp powerup) {
				...
				try {
					...
					PlayerAIBase.STATUS puStatus = PlayerAIBase.STATUS
							.valueOf(xml.getRootElement().attribute("status")
									.getValue());
					String puGuid = xml.getRootElement().attribute("played-by") != null ? xml
							.getRootElement().attribute("played-by").getValue()
							: myGuid;
					...
					PowerUp cardPlayed = PowerUp.GenerateFlyweight(xml
							.getRootElement().element("card"), brain
							.getCompanies(), brain.getPassengers(), brain
							.getPlayers());

					if (log.isInfoEnabled())
						log.info(plyrPowerUp.getName() + " " + puStatus
								+ " on " + cardPlayed);
					...
					if (cardPlayed.equals(cardLastPlayed)
						...
						UpdateCards(xml.getRootElement().element("cards-deck")
								.elements("card"), brain.getPowerUpDeck(),
								brain.getPowerUpHand());
						// delete any not in drawn
						UpdateCards(xml.getRootElement().element("cards-hand")
								.elements("card"), brain.getPowerUpHand(), null);
					}
				...
				// may be here because re-started and got this message before
				// the re-send of setup.
				if (net.windward.Windwardopolis2.DotNetToJavaStringHelper
						.isNullOrEmpty(myGuid)) {
					return;
				}
				...
				try {
					...
					PlayerAIBase.STATUS status = PlayerAIBase.STATUS
							.valueOf(xml.getRootElement().attribute("status")
									.getValue());
					Attribute attr = xml.getRootElement().attribute(
							"player-guid");
					String guid = attr != null ? attr.getValue() : myGuid;

					Player.UpdateFromXml(brain.getCompanies(), brain
							.getPlayers(), brain.getPassengers(), xml
							.getRootElement().element("players"));
					Passenger.UpdateFromXml(brain.getCompanies(), brain
							.getPlayers(), brain.getPassengers(), xml
							.getRootElement().element("passengers"));
						...
						for (String stepOn : path) {
							...
							if (pos > 0)
								plyrStatus
										.getLimo()
										.getPath()
										.add(new Point(Integer.parseInt(stepOn
												.substring(0, pos)), Integer
												.parseInt(stepOn
														.substring(pos + 1))));
						}
					}
				...
				TRAP.trap();
				String msg = String
						.format("ERROR: bad message (XML) from server - root node %1$s",
			...
			if (turnTime > 800) {
				System.out.println("WARNING - turn took " + turnTime / 1000
						+ " seconds");

			}
		} catch (RuntimeException ex) {
			System.out.println(String.format(
					"Error on incoming message. Exception: %1$s", ex));
			ex.printStackTrace();
			log.error("Error on incoming message.", ex);
		}
	}

	private void UpdateCards(java.util.List<Element> elements,
			java.util.List<PowerUp> cardList, java.util.List<PowerUp> hand) {
		...
		while (it.hasNext()) {
			...
			deck.add(PowerUp.GenerateFlyweight(current, brain.getCompanies(),
					brain.getPassengers(), brain.getPlayers()));
		}
	...
	private void PlayerOrdersEvent(String order,
		...
		try {
			...
			if (path.size() > 0) {
				...
				for (Point ptOn : path) {
					buf.append(String.valueOf(ptOn.x) + ','
							+ String.valueOf(ptOn.y) + ';');
				}
			...
			log.error("PlayerOrderEvent( " + order + ", ...) threw Exception: "
					+ e.getMessage());
		}
	}

	private void PlayerPowerSend(PlayerAIBase.CARD_ACTION action,
			PowerUp powerup) {
		...
		elemCard.add(DocumentHelper.createAttribute(elemCard, "card", powerup
				.getCard().name()));

		if (powerup.getCompany() != null)
			elemCard.add(DocumentHelper.createAttribute(elemCard, "company",
					powerup.getCompany().getName()));
		if (powerup.getPassenger() != null)
			elemCard.add(DocumentHelper.createAttribute(elemCard, "passenger",
					powerup.getPassenger().getName()));
		if (powerup.getPlayer() != null)
			elemCard.add(DocumentHelper.createAttribute(elemCard, "player",
					powerup.getPlayer().getName()));
	...
	public final void ConnectionLost(Exception ex) throws IOException,
			InterruptedException {

		System.out
				.println("Lost our connection! Exception: " + ex.getMessage());
		...
		while (true) {
			try {
				...
				System.out.println("Re-connection fails! Exception: "
						+ e.getMessage());
				Thread.sleep(delay);
				delay += 500;
			}
